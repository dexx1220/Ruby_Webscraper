...
#Method to find highest/lowest priced apartment(s)
def hi_low_price(choice='hi')
  has_price = 0
...
#Method to find apartment(s) with largest/smallest sqft.
def hi_low_area(choice='hi')
  has_area = 0
  has_area = $postings.select{|p| p[1][/\d+(?=ft)/].to_i >= 1}
  max_post = has_area.max {|a,b| a[1][/\d+(?=ft)/].to_i <=> b[1][/\d+(?=ft)/].to_i}
...
#Method to find apartment(s) with most/least rooms
def hi_low_rooms(choice='hi')
  has_info = 0
